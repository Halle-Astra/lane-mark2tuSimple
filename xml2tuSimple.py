...
def xmls2tuSimple(annotation_files, config):#coorY: list or dict):
    ...
    for annotation_file in annotation_files:
        ...
        jsonify_data_tuSimple(data_tuSimple, config["add_prefix"])
...
def jsonify_data_tuSimple(data_tuSimple, folder_prefix='tuSimple_'):
    """jsonify the file which should be dumped. **The path of target file must not be placed under the root if you are using Windows system, eg. C:, D:"""
    aim_path = data_tuSimple['raw_file'].replace(".xml", ".json")
    folder, file_name = os.path.split(aim_path)
    folder = folder_prefix + folder
    os.makedirs(folder, exist_ok=True)
    aim_path = os.path.join(folder, file_name)
    f = open(aim_path, 'w')
...
def getConfig(args):
    ...
    for key in args.__dict__:
        json_content[key] = args.__getattribute__(key)
...
if __name__ == "__main__":
    ...
    args.add_argument("--add_prefix", "-pre", type=str,
                      default="tuSimple_",
                      help="The prefix string for the folder generated by this script.")
